...
plot(dat, style = "ggplot", pool.replicate = FALSE)
plot(dat, concentration = 4.36, addlegend = TRUE, style = "ggplot",
     pool.replicate = FALSE)

# (5) represente the survival rate as a function of concentration
plotDoseResponse(dat, target.time = 21, style = "ggplot")

# (6) check information on the experimental design
summary(dat)

# (7) fit an time-exposure-response model at
fit <- survFitTKTD(dat, quiet = TRUE)
summary(fit)
```

```{r, cache=TRUE}
plot(fit, style = "ggplot", adddata = TRUE)
```
